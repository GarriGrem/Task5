...
public class Task5 {
    ...
    public static boolean validateCard (long number) {
        ...
        if (10 - Character.getNumericValue(str.charAt(str.length() - 1)) == control)
            return true;
        else
            return false;
    }
    ...
    public static String numToEng (int num) {
        String[] list1 = {"one", "two", "three", "four", "five", "six", "seven", "eight",
        "nine", "ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen",
        "seventeen", "eighteen", "nineteen", ""};
        String[] list2 = {"twenty", "thirty", "forty", "fifty", "sixty", "seventy",
        "eighty", "ninety"};
        ...
        if (num == 0)
            return "zero";
        if (num % 100 == 0)
            return list1[num / 100 - 1] + " hundred ";
        if (num % 10 == 0 && num > 19)
            return list2[num / 10 - 2];
        if (num / 100 != 0 && num % 100 == 0)
            result = list1[num / 100 - 1] + " hundred and ";
        else if (num / 100 != 0)
            result = list1[num / 100 - 1] + " hundred ";
        else
            result = "";
        if (num % 100 <= 19)
            result += list1[num % 100 - 1];
        else
            result += list2[num % 100 / 10 - 2] + " " + list1[num % 10 - 1];
        return result;
    }
